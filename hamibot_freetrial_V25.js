...
var LOG_OK="";
for(var _li=0;_li<LOG_CANDS.length;_li++){try{files.write(LOG_CANDS[_li],"=== V25 start "+new Date().toLocaleString()+" ==="+"\n");LOG_PATH=LOG_CANDS[_li];LOG_OK=LOG_PATH;break;}catch(_le){}}
...
toast("V25 start log:"+LOG_OK);
L("V25 start log:"+LOG_OK);
...
function ensureMeishi(){
  ...
  for(var r=0;r<3;r++){
    ...
    if(list.length==0){
      ...
      } else { L("no quanfenlei, stop (V25: no blind tap)"); return false; }
      sleep(2500);
      list=[];
...
function guardBack(tag){if(inFreeList()){L(tag+": still in free list,skip back");return true;}back();sleep(2500);if(inFreeList()){L(tag+": back to list ok");return true;}L(tag+": 2nd back");back();sleep(2500);var ok=inFreeList();L(tag+": after 2nd back inFree="+ok);return ok;}function doBaoMing(){
  ...
  // V25 detail recheck (U2 parity): detail distance + already-registered
  try{
    var _dt="";
