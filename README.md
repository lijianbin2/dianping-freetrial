...
- 手机上只传一个：hamibot_免费试页开始_V19.js（从免费试列表页开始，不碰首页）。
- 点运行后必须先看到 V19 start，再看到 V19 auto ok:true；只看到前者=无障碍没开。
- 先跑 hamibot_冒烟测试.js：冒烟都不弹=Hamibot 环境问题，不是脚本问题。
- V19相对V18只改启动：auto.waitFor无限等改成15秒超时明示退出；报名逻辑与V18一致。
...
- 运行：手动停在免费试列表页 → 粘贴 V11 运行 → 切美食 → 价值>100元且<20km → 我要报名→确认报名→完成→back→下一家，直到等级不够结束。
