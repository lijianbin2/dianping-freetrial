...
var LOG_OK="";
for(var _li=0;_li<LOG_CANDS.length;_li++){try{files.write(LOG_CANDS[_li],"=== V23 start "+new Date().toLocaleString()+" ==="+"\n");LOG_PATH=LOG_CANDS[_li];LOG_OK=LOG_PATH;break;}catch(_le){}}
...
toast("V23 start log:"+LOG_OK);
L("V23 start log:"+LOG_OK);
...
function ensureMeishi(){
  ...
  for(var r=0;r<3;r++){
    ...
    if(list.length==0){
      ...
      } else { L("no quanfenlei, stop (V23: no blind tap)"); return false; }
      sleep(2500);
      list=[];
...
function tryOpenQualifiedOnce(){
  ...
  for(var a=0;a<ax.length;a++){
    var y=ax[a].cy;
    var win=[];
    for(var i=0;i<all.length;i++){if(Math.abs(all[i].cy-y)<200)win.push(all[i]);}
    ...
    if(val>100&&dist>=0&&dist<20){
      L("hit"+val+"yuan"+dist+"km y="+y); if(y<600){L("skip y<600 fastfilter guard"); continue;}
...
function guardBack(tag){if(inFreeList()){L(tag+": still in free list,skip back");return true;}back();sleep(2500);if(inFreeList()){L(tag+": back to list ok");return true;}L(tag+": 2nd back");back();sleep(2500);var ok=inFreeList();L(tag+": after 2nd back inFree="+ok);return ok;}function doBaoMing(){
  ...
  // V23 detail recheck (U2 parity): detail distance + already-registered
  try{
    var _dt="";
