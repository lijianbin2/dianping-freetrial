// V16 2026-09-13 minimal: 手动进入免费试列表页后开始,不再找首页入口. 起手ensureMeishi,严校验.
auto.waitFor();
...
function ensureMeishi(){
  ...
  for(var r=0;r<3;r++){
    ...
    if(list.length==0){
      log("popup closed, tap quanfenlei tab");
      var qs=null;try{qs=text("全部分类").find();}catch(e){}
      if(qs&&qs.length>0){
        try{qs.sort(function(x,y){return x.bounds().centerY()-y.bounds().centerY();});}catch(e){}
        try{var q0=qs[0];log("tap quanfenlei y="+q0.bounds().centerY());try{var qb=q0.bounds();click(qb.centerX(),qb.centerY());}catch(e2){try{q0.click();}catch(e3){}}}catch(e){}
      } else { log("no quanfenlei text, tap 481,1529"); var sx=device.width/1280,sy=device.height/2772; click(481*sx,1529*sy); }
      sleep(2500);
      list=[];
    ...
    log("meishi cands="+list.length);
    if(list.length==0){log("no meishi, retry");sleep(1500);continue;}
    try{list.sort(function(x,y){return y.bounds().centerY()-x.bounds().centerY();});}catch(e){}
    for(var mi=0;mi<list.length;mi++){
      try{var m=list[mi];log("tap meishi y="+m.bounds().centerY());try{var mb=m.bounds();click(mb.centerX(),mb.centerY());}catch(e2){try{m.click();}catch(e3){}}}catch(e){}
      sleep(3000);
      if(isMeishiTab()){toast("to meishi ok");return true;}
      if(!inFreeList()){toast("jumped out,stop");log("jumped out, stop no back");return false;}
    }
  }
  toast("not in meishi");log("not in meishi after 3 tries");return false;
}
function tryOpenQualifiedOnce(){
  ...
  for(var a=0;a<ax.length;a++){
    ...
    if(val>100&&dist>=0&&dist<20){
      ...
      sleep(2500);
      if(inFreeList()){log("open fail still in list, try next");continue;}
...
function guardBack(tag){if(inFreeList()){log(tag+": still in free list,skip back");return true;}back();sleep(2500);if(inFreeList()){log(tag+": back to list ok");return true;}log(tag+": 2nd back");back();sleep(2500);var ok=inFreeList();log(tag+": after 2nd back inFree="+ok);return ok;}function doBaoMing(){
  log("doBaoMing start pkg="+currentPackage());
  var want=text("我要报名").findOne(6000);
  if(!want){
    log("doBaoMing: no 我要报名,back");
    try{var w2=textContains("我要报名").findOne(2000); if(w2){log("found contains version"); clickUpClickable(w2);} else { if(inFreeList()){log("tap missed,NO back");return "no_entry";} if(!guardBack("no_entry"))return "lost"; return "no_entry"; }}catch(e){ if(inFreeList()){log("tap missed,NO back");return "no_entry";} if(!guardBack("no_entry"))return "lost"; return "no_entry"; }
  } else {
...
while(true){
  ...
  if(r=="lost"){ toast("page lost,stop"); log("LOST,stop"); break; } if(r=="level_buzu"){ toast("等级不够,结束"); break; }
  if(r=="no_entry"||r=="no_confirm"){failStreak++;log("failStreak="+failStreak+" swipe past bad card");swipe(device.width/2,device.height*0.72,device.width/2,device.height*0.28,700);sleep(1800);if(failStreak>=3){swipe(device.width/2,device.height*0.75,device.width/2,device.height*0.25,800);sleep(1800);}}else{failStreak=0;}
  sleep(1500);
}
